	...
	<div class="row">
		<div class="pull-left" style="padding: 20px; margin-right: -1px;border-right: 1px solid #ddd;">
			...
			<div class="thumbnail img-circle" style="width: 140px; height: 140px;">
      			<div class="bg-image cover" style="background-image: url(<?php echo $this->data->goods_pic?>);">
      			...
      			<input id="ajax_image" type="hidden" name="ajax_image" value="<?php echo $this->data->goods_pic?>">
      			<input id="goods_id" type="hidden" name="goods_id" value="<?php echo $this->data->goods_id?>">
      		</div>
		...
		<div style="margin-left: 180px;width: auto; border-left: 1px solid #ddd; padding: 20px;">	
			<div class="form-group"><label>分店名称</label> 
    			<select name="store_id"  class="form-control">
    			  <option value="0" <?php if($this->data->store_id == 0){ echo 'selected';}?>>无</option>
    			  <?php $stores = \DB\Mall\Store::fetch();?>
    			  <?php foreach ($stores as $store):?>
				  <option value="<?php echo $store->store_id?>" <?php if($this->data->store_id == $store->store_id){ echo 'selected';}?>><?php echo $store->store_name?></option>
				  <?php endforeach;?>
				</select>
			</div>
			<div class="form-group"><label>分类名称</label> 
    			<select name="category_id"  class="form-control">
				  ...
				  <option value="<?php echo $c->category_id?>" <?php if($this->data->category_id == $c->category_id){ echo 'selected';}?>><?php echo $c->category_name?></option>
				  <?php endforeach;?>
				</select>
			</div>
      		<div class="form-group"><label>商品名称</label> 
            <input type="text" class="form-control" id="goods_name" name="goods_name" value="<?php echo $this->data->goods_name?>" placeholder="goods_name">
			</div>
			<div class="row">
				<div class="col-md-6">
		            <div class="form-group"><label>销售价格</label> 
		            <input type="number" class="form-control" id="goods_discount_price" name="goods_discount_price" value="<?php echo $this->data->goods_discount_price?>">
		            </div>
				</div>
				<div class="col-md-6">
					<div class="form-group"><label>商品原价</label> 
		            <input type="number" class="form-control" id="goods_original_price" name="goods_original_price" value="<?php echo $this->data->goods_original_price?>">
		            </div>
				</div>
			</div>
			<div>
				<div class="col-md-4" style="padding-left: 0px;">
		            ...
		            <input type="number" class="form-control" id="goods_order" name="goods_order" value="<?php echo $this->data->goods_order?>">
		            </div>
				</div>
				<div class="col-md-4" style="padding-right: 0px;" >
					<div class="form-group"><label>是否推荐</label> 
						<select name="is_recommend"  class="form-control">
						  <option value="0" <?php if($this->data->is_recommend == '0'){ echo 'selected';}?>>否</option>
						  <option value="1" <?php if($this->data->is_recommend == '1'){ echo 'selected';}?>>是</option>
						</select>
		            </div>
				</div>
				<div class="col-md-4">
					<div class="form-group"><label>推荐星级</label> 
						<select name="count_star"  class="form-control">
						  <option value="0" <?php if($this->data->count_star == 0){ echo 'selected';}?>>0</option>
						  <option value="1" <?php if($this->data->count_star == 1){ echo 'selected';}?>>1</option>
						  <option value="2" <?php if($this->data->count_star == 2){ echo 'selected';}?>>2</option>
						  <option value="3" <?php if($this->data->count_star == 3){ echo 'selected';}?>>3</option>
						  <option value="4" <?php if($this->data->count_star == 4){ echo 'selected';}?>>4</option>
						  <option value="5" <?php if($this->data->count_star == 5){ echo 'selected';}?>>5</option>
						</select>
		            </div>
				</div>
			</div>
			<div class="form-group"><label>商品摘要</label> 
            <textarea type="textarea" class="form-control" id="goods_profile" name="goods_profile" placeholder="预览的描述信息" rows="6"><?php echo $this->data->goods_profile?></textarea>
            </div>
            <div class="form-group"><label>商品简介</label> 
            <textarea type="textarea" class="form-control" id="goods_content" name="goods_content" placeholder="预览的描述信息"><?php echo $this->data->goods_content?></textarea>
            </div>
...
$(document).ready(function(){
    ...
    var filechange = function(){
        $.ajaxFileUpload({
            ...
            {
                $(".bg-image.cover").css("background-image", "url("+data.imgurl+")");
